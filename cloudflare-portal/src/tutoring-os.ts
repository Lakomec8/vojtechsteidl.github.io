...
const LEAD_TRIAL_API_PATH = "/student-portal/api/admin/tutoring/leads/trial";
...
type LeadRow = {
  ...
  trial_status: "scheduled" | "completed" | "converted" | "not_converted" | "cancelled" | null;
...
async function dashboardData(env: Env): Promise<{
  ...
  const [students, lessons, events, expenses, leads, sync] = await Promise.all([
    ...
    env.DB.prepare(`SELECT l.id, l.source, l.title, l.subject, l.source_url, l.score, l.status, l.first_seen_at,
                           t.status AS trial_status, t.scheduled_week, t.prospect_label
                      FROM tutoring_leads AS l
                      LEFT JOIN tutoring_lead_trials AS t ON t.lead_id = l.id
...
function renderDashboard(data: Awaited<ReturnType<typeof dashboardData>>, email: string): Response {
  ...
  const leadTrials = leadEligible.filter((lead) => ["scheduled", "completed"].includes(lead.trial_status || ""));
  const leadTrialRate = leadEligible.length ? Math.round((leadTrials.length / leadEligible.length) * 100) : 0;
  ...
  const leadWeeks: Array<{ key: string; total: number; trials: number; won: number }> = [];
  const leadWeekMap = new Map<string, { total: number; trials: number; won: number }>();
  for (const lead of leadEligible) {
    const key = weekStart(lead.first_seen_at.slice(0, 10));
    const item = leadWeekMap.get(key) || { total: 0, trials: 0, won: 0 };
    item.total += 1;
    if (["scheduled", "completed", "converted"].includes(lead.trial_status || "")) item.trials += 1;
  ...
  for (let index = 0; index < 8; index++) {
    ...
    const item = leadWeekMap.get(key) || { total: 0, trials: 0, won: 0 };
    leadWeeks.push({ key, total: item.total, trials: item.trials, won: item.won });
  ...
  const leadFunnel = [
    ...
    ["Zkušební hodina", leadTrials.length],
    ["Uzavřené", leadClosed],
  ...
  const leadWeekBars = leadWeeks.map((item) => {
    ...
    const trialHeight = item.trials ? Math.max(7, Math.round((item.trials / maxLeadWeek) * 130)) : 2;
    const wonHeight = item.won ? Math.max(7, Math.round((item.won / maxLeadWeek) * 130)) : 2;
    const trialRate = item.total ? Math.round((item.trials / item.total) * 100) : 0;
    return `<div class="lead-week-column"><div class="lead-week-values"><span>${item.total}</span><span>${item.trials}</span><span>${item.won}</span></div><div class="lead-week-bars"><i class="all" style="height:${totalHeight}px"></i><i class="trial-bar" style="height:${trialHeight}px"></i><i class="won" style="height:${wonHeight}px"></i></div><strong>${esc(weekLabel(item.key))}</strong><small>${trialRate}% trial</small></div>`;
  }).join("");

  const leadRows = leads.slice(0, 24).map((lead) => `
    <tr data-lead-id="${esc(lead.id)}">
      <td><strong>${esc(lead.prospect_label ? `${lead.prospect_label} · ${lead.title}` : lead.title)}</strong><small>${esc(lead.subject || lead.source)}</small></td>
      <td><span class="score-pill">${esc(lead.score)}/100</span></td>
      <td>${lead.trial_status && ["scheduled","completed"].includes(lead.trial_status)
        ? '<span class="pill trial">Zkušební domluvena</span>'
        : `<span class="pill lead-${esc(lead.status)}">${esc(leadStatusLabel(lead.status))}</span>`}
        ${lead.scheduled_week ? `<small>Týden od ${esc(dateLabel(lead.scheduled_week))}</small>` : ""}</td>
      ...
      <td class="lead-actions-cell">
        ...
        <button class="mini-action trial-action" type="button" data-trial-action="${lead.trial_status ? "clear" : "scheduled"}">${lead.trial_status ? "Zrušit trial" : "Zkušební"}</button>
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;background-color:var(--paper);background-image:linear-gradient(rgba(62,63,57,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(62,63,57,.055) 1px,transparent 1px);background-size:36px 36px}
    button,a{font:inherit}.shell{width:calc(100% - 24px);max-width:none;margin:12px auto;display:grid;grid-template-columns:240px minmax(0,1fr);min-height:calc(100vh - 24px);border:1px solid rgba(151,148,137,.55);border-radius:24px;background:rgba(248,247,241,.88);box-shadow:var(--shadow);overflow:hidden;backdrop-filter:blur(16px)}
    .sidebar{padding:28px 20px;background:rgba(231,229,220,.76);border-right:1px solid var(--line);display:flex;flex-direction:column;gap:28px}.brand{display:flex;gap:12px;align-items:center;padding:0 7px}.brand-mark{width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:var(--mint);box-shadow:inset 0 1px rgba(255,255,255,.8),0 7px 14px rgba(56,129,101,.18);font-weight:900}.brand strong{display:block;font-size:15px}.brand span{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.nav{display:grid;gap:7px}.nav button{display:flex;align-items:center;gap:11px;width:100%;border:0;background:transparent;color:#5d5f58;padding:11px 12px;border-radius:13px;text-align:left;cursor:pointer;font-weight:700}.nav button:hover{background:rgba(255,255,255,.55)}.nav button.active{color:var(--ink);background:var(--surface);box-shadow:var(--shadow-soft)}.nav svg{width:18px;height:18px}.sync-card{margin-top:auto;padding:15px;border-radius:18px;background:var(--surface);border:1px solid var(--line);box-shadow:var(--shadow-soft)}.sync-head{display:flex;align-items:center;gap:8px;font-weight:800;font-size:13px}.sync-dot{width:9px;height:9px;border-radius:50%;background:var(--mint-strong);box-shadow:0 0 0 4px var(--mint-soft)}.sync-card p{font-size:12px;color:var(--muted);line-height:1.5;margin:10px 0 0}.account{padding:9px 7px 0;border-top:1px solid var(--line);font-size:11px;color:var(--muted);overflow:hidden;text-overflow:ellipsis}
    main{padding:32px 38px 48px;min-width:0}.topbar{display:flex;justify-content:space-between;gap:24px;align-items:flex-start;margin-bottom:24px}.eyebrow{font-size:11px;font-weight:900;letter-spacing:.13em;text-transform:uppercase;color:#557765}.topbar h1{font-size:clamp(36px,3.4vw,64px);line-height:.98;letter-spacing:-.055em;margin:7px 0 8px}.topbar p{margin:0;color:var(--muted);font-size:14px}.top-actions{display:flex;gap:9px;align-items:center}.button{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--line);border-radius:999px;padding:10px 15px;color:var(--ink);background:var(--surface);text-decoration:none;font-weight:800;font-size:13px;box-shadow:var(--shadow-soft)}.button.primary{background:var(--mint);border-color:#83dcb9}.button:hover{transform:translateY(-1px)}
    .view{display:none}.view.active{display:block}.kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;margin-bottom:16px}.card{background:rgba(251,250,246,.93);border:1px solid var(--line);border-radius:22px;box-shadow:var(--shadow-soft)}.kpi{padding:22px;min-height:154px;display:flex;flex-direction:column}.kpi-top{display:flex;justify-content:space-between;align-items:center}.kpi-label{font-size:13px;color:var(--muted);font-weight:800}.icon-box{width:34px;height:34px;border-radius:11px;background:var(--surface-2);display:grid;place-items:center}.icon-box.mint{background:var(--mint-soft)}.kpi strong{font-size:clamp(27px,3vw,42px);letter-spacing:-.045em;margin-top:auto}.kpi small{color:var(--muted);margin-top:4px;font-size:12px}.delta{color:#347c61;font-weight:800}.content-grid{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(360px,.85fr);gap:16px;margin-bottom:16px}.section{padding:24px}.section-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}.section h2{margin:0;font-size:21px;letter-spacing:-.025em}.section-head p{margin:5px 0 0;color:var(--muted);font-size:13px}.chip{border:1px solid var(--line);background:var(--surface-2);padding:7px 11px;border-radius:999px;font-size:11px;font-weight:800}.chart{height:220px;display:flex;align-items:flex-end;gap:16px;padding:28px 8px 0;border-bottom:1px solid var(--line);overflow-x:auto;overflow-y:hidden;background-image:linear-gradient(rgba(62,63,57,.06) 1px,transparent 1px);background-size:100% 48px}.bar-column{height:100%;flex:1 1 88px;min-width:76px;display:flex;align-items:center;justify-content:flex-end;flex-direction:column;position:relative}.bar-column>span{font-size:11px;font-weight:800;margin-bottom:7px}.bar{width:min(58px,70%);border-radius:14px 14px 4px 4px;background:linear-gradient(180deg,var(--mint),#78d8b4);box-shadow:inset 0 1px rgba(255,255,255,.75),0 8px 16px rgba(67,140,111,.2)}.bar-column strong{font-size:12px;margin-top:9px;text-transform:capitalize}.bar-column small{font-size:10px;color:var(--muted)}.split-list{display:grid;gap:14px}.split-row{display:grid;grid-template-columns:72px minmax(80px,1fr) 70px;gap:9px;align-items:center;font-size:12px}.split-row span{font-weight:700}.split-row strong{text-align:right}.track{height:9px;border-radius:99px;background:var(--surface-2);overflow:hidden}.track i{display:block;height:100%;border-radius:inherit;background:var(--mint-strong)}.track i.blue{background:var(--blue)}.track i.amber{background:var(--amber)}.track i.violet{background:var(--violet)}.track i.stone{background:#aaa99f}
    .wide-card{padding:24px;overflow:auto}.table-head{display:flex;justify-content:space-between;gap:15px;align-items:center;margin-bottom:14px}.table-head h2{margin:0;font-size:21px}.search{display:flex;align-items:center;gap:8px;padding:9px 13px;border:1px solid var(--line);border-radius:999px;background:var(--surface);color:var(--muted)}.search input{border:0;outline:0;background:transparent;width:150px;color:var(--ink)}table{width:100%;border-collapse:collapse;min-width:780px}th{text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);padding:10px 11px;border-bottom:1px solid var(--line)}td{padding:15px 12px;border-bottom:1px solid rgba(213,210,200,.7);font-size:14px;vertical-align:middle}tr:last-child td{border-bottom:0}td small{display:block;color:var(--muted);font-size:10px;margin-top:3px}td.money{font-weight:900}.avatar{display:inline-grid;place-items:center;width:29px;height:29px;border-radius:10px;background:var(--mint);margin-right:9px;box-shadow:inset 0 1px rgba(255,255,255,.7)}.avatar.blue{background:var(--blue)}.avatar.amber{background:var(--amber)}.avatar.violet{background:var(--violet)}.avatar.stone{background:#c9c7be}.pill{display:inline-flex;align-items:center;border-radius:99px;padding:5px 9px;font-size:10px;font-weight:900;background:var(--surface-2);white-space:nowrap}.pill.success{background:var(--mint-soft);color:#2f7258}.pill.planned{background:#e5eef3;color:#4f7187}.pill.muted{color:var(--muted)}.status-dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--mint-strong);margin-right:8px}.status-dot.baseline{background:var(--amber)}
    ...
    .tax-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.funnel-list{display:grid;gap:13px}.funnel-row{display:grid;grid-template-columns:92px minmax(80px,1fr) 34px;gap:10px;align-items:center;font-size:12px}.funnel-row span{font-weight:750}.funnel-row strong{text-align:right}.lead-week-chart{height:210px;display:flex;align-items:flex-end;gap:12px;padding:24px 5px 0;border-bottom:1px solid var(--line);overflow-x:auto;background-image:linear-gradient(rgba(62,63,57,.06) 1px,transparent 1px);background-size:100% 44px}.lead-week-column{height:100%;flex:1 1 92px;min-width:84px;display:flex;align-items:center;justify-content:flex-end;flex-direction:column}.lead-week-values{display:flex;gap:12px;font-size:10px;font-weight:850;margin-bottom:6px}.lead-week-bars{height:132px;display:flex;align-items:flex-end;gap:5px}.lead-week-bars i{display:block;width:25px;border-radius:10px 10px 3px 3px}.lead-week-bars .all{background:var(--blue)}.lead-week-bars .trial-bar{background:var(--violet)}.lead-week-bars .won{background:var(--mint-strong)}.lead-week-column strong{font-size:11px;margin-top:8px}.lead-week-column small{font-size:9px;color:var(--muted)}.score-pill{display:inline-flex;padding:5px 8px;border-radius:99px;background:#e5eef3;color:#4f7187;font-size:10px;font-weight:900}.pill.lead-won{background:var(--mint-soft);color:#2f7258}.pill.lead-lost,.pill.lead-ignored{background:#f3e6e2;color:#8b5f55}.pill.lead-new{background:#f6edce;color:#6f5b1f}.pill.trial{background:#efe8fb;color:#684f91}.lead-actions-cell{display:flex;gap:5px;flex-wrap:wrap}.mini-action{border:1px solid var(--line);background:var(--surface);color:var(--ink);border-radius:999px;padding:6px 10px;text-decoration:none;font:inherit;font-size:10px;font-weight:850;cursor:pointer}.mini-action.success-action{background:var(--mint-soft);border-color:#b8e8d5}.mini-action.trial-action{background:#f3eefb;border-color:#d9cbed}.tax-hero{padding:25px;min-height:280px;display:flex;flex-direction:column}.tax-number{font-size:clamp(44px,6vw,78px);letter-spacing:-.065em;margin:28px 0 4px}.tax-breakdown{margin-top:auto;display:grid;gap:11px}.tax-row{display:flex;justify-content:space-between;border-top:1px solid var(--line);padding-top:11px;font-size:13px}.tax-row span{color:var(--muted)}.note{padding:14px;border-radius:15px;background:#f6edce;color:#6f5b1f;font-size:12px;line-height:1.55}.empty{padding:28px;text-align:center;color:var(--muted)}
    @media(min-width:1500px){.nav button{font-size:14px}.brand strong{font-size:16px}.brand span{font-size:12px}.button{font-size:14px}.chip{font-size:12px}.funnel-row{font-size:13px}.tax-row{font-size:14px}}
...
<body>
    ...
    <main>
      ...
      <section class="view active" id="overview">
        <div class="kpis">
          ...
          <article class="card kpi"><div class="kpi-top"><span class="kpi-label">Lead → zkušební hodina</span><span class="icon-box mint">✓</span></div><strong>${leadTrialRate} %</strong><small>${leadTrials.length} domluvené · ${leadWon.length} potvrzení klienti</small></article>
        </div>
        ...
        <div class="content-grid">
          <article class="card section"><div class="section-head"><div><h2>Lead funnel</h2><p>Aktuální stav akvizice ze zachycených relevantních poptávek</p></div><a class="chip" href="#leads" data-target="leads">Detail leadů</a></div><div class="funnel-list">${leadFunnel}</div><div class="note" style="margin-top:20px">Lead → zkušební hodina: <strong>${leadTrialRate} %</strong> · lead → klient: <strong>${leadConversion} %</strong> · odpovězenost: <strong>${leadReplyRate} %</strong> · průměrné skóre: <strong>${averageLeadScore}/100</strong>.</div></article>
      ...
      <section class="view" id="leads">
        <div class="kpis">
          ...
          <article class="card kpi"><span class="kpi-label">Zkušební hodiny</span><strong>${leadTrials.length}</strong><small>${leadTrialRate} % relevantních leadů</small></article>
          <article class="card kpi"><span class="kpi-label">Potvrzení klienti</span><strong>${leadWon.length}</strong><small>close rate ${leadCloseRate} %</small></article>
        </div>
        <div class="content-grid">
          <article class="card section"><div class="section-head"><div><h2>Úspěšnost leadů po týdnech</h2><p>Modře leady · fialově zkušební hodiny · zeleně potvrzení klienti</p></div><span class="chip">8 týdnů</span></div><div class="lead-week-chart">${leadWeekBars}</div></article>
          <article class="card section"><div class="section-head"><div><h2>Funnel</h2><p>Od relevantní poptávky po klienta</p></div><a class="chip" href="/student-portal/admin/tutoring/leads/">Lead inbox</a></div><div class="funnel-list">${leadFunnel}</div><div class="note" style="margin-top:20px">Lead → zkušební hodina <strong>${leadTrialRate} %</strong>. Zkušební hodina se ještě nepočítá jako získaný klient; konverzi na klienta započítáme až po potvrzení další spolupráce.</div></article>
        </div>
  ...
  <script>
    ...
    document.querySelectorAll('[data-trial-action]').forEach(button=>button.addEventListener('click',async()=>{const row=button.closest('[data-lead-id]');if(!row)return;button.disabled=true;try{const response=await fetch('/student-portal/api/admin/tutoring/leads/trial',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},body:JSON.stringify({id:row.dataset.leadId,action:button.dataset.trialAction})});if(!response.ok)throw new Error('trial update failed');location.hash='leads';location.reload()}catch{button.disabled=false}}));
  </script>
</body>
</html>`);
}
...
async function updateLeadTrial(request: Request, env: Env): Promise<Response> {
  await requireAdmin(request, env);
  if (request.method !== "POST") return plain("Method not allowed", 405);
  if (request.headers.get("x-requested-with") !== "XMLHttpRequest") return plain("Missing request marker", 403);
  if (!request.headers.get("content-type")?.toLowerCase().includes("application/json")) return plain("JSON required", 415);
  const payload = await request.json<{ id?: string; action?: string }>();
  const id = String(payload.id || "").trim();
  const action = String(payload.action || "").trim();
  if (!id || !["scheduled", "clear"].includes(action)) return json({ error: "Invalid trial update." }, 400);
  const lead = await env.DB.prepare("SELECT id FROM tutoring_leads WHERE id = ?1").bind(id).first<{ id: string }>();
  if (!lead) return json({ error: "Lead not found." }, 404);
  if (action === "clear") {
    await env.DB.prepare("DELETE FROM tutoring_lead_trials WHERE lead_id = ?1").bind(id).run();
  } else {
    const now = new Date();
    const day = now.getUTCDay() || 7;
    const nextMonday = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + (8 - day)));
    const scheduledWeek = nextMonday.toISOString().slice(0, 10);
    await env.DB.prepare(`INSERT INTO tutoring_lead_trials (lead_id, scheduled_week, status)
      VALUES (?1, ?2, 'scheduled')
      ON CONFLICT(lead_id) DO UPDATE SET scheduled_week=excluded.scheduled_week, status='scheduled', updated_at=CURRENT_TIMESTAMP`)
      .bind(id, scheduledWeek).run();
    await env.DB.prepare("UPDATE tutoring_leads SET status = 'replied' WHERE id = ?1 AND status IN ('new','reviewed')")
      .bind(id).run();
  }
  return json({ ok: true });
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === LEAD_TRIAL_API_PATH) return updateLeadTrial(request, env);
